...
import type { AttributeSelector, BookmarkInternalData, ExplorerHiderSettings, Hidden} from "./interface";
...
export class RulesCompiler {
	...
	/**
	 * Simple colored folder add data-path attribute only to the first root folder
	 * @param path {string}
	 * @param selectorChar
	 * @private
	 * @returns {string} " " or .tree-item.nav-folder[data-path="${path}"],
	 */
	private spfCompat(path: string, selectorChar: AttributeSelector | "^"): string {
		const defaultStr = " ";
		const length = path.split("/").length;
		if (!this.settings.compatSpf || length >1) return defaultStr;
		return ` .tree-item.nav-folder[data-path${selectorChar}="${path}"], `;
	}
	...
	createRuleFolderTitle(snippet: Hidden): string | undefined {
		...
		const spfCompat = this.spfCompat(path, selectorChar);
	...
	private formatCSS(css: string) {
		const minify = !this.settings.useSnippets;
	...
	compileNavRules() {
		...
		if (rule.length > 0)
			return `${rule.trimEnd().replace(/,$/gi, "")} { display: none; }`;
	...
	createRules() {
		return this.formatCSS(`${this.compileNavRules()}\n${this.compileBookmarksRules()}`);
	}
